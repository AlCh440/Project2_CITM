...
{
	button = (GuiButton*)app->guiManager->pn_inventory->CreateGuiButton(app->guiManager->pn_inventory->GetNumberOfButtons(), app->guiManager, app->guiManager->pn_inventory, { (position.x + (222)) / app->win->GetScale(), ( position.y + (170)) / app->win->GetScale(), 44, 44 }, "", app->fonts->menuButtonFont, app->fonts->c_Menus);
